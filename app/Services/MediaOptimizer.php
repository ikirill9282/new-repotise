...
class MediaOptimizer
{
    ...
    protected function optimizeImage(string $absolutePath, array $options = []): array
    {
        ...
        try {
            ...
            $isPNG = strtolower($extension) === 'png';
            ...
            // Для больших PNG файлов применяем еще более агрессивную оптимизацию
            if ($isLargeFile && $isPNG) {
                // PNG файлы могут быть очень большими, уменьшаем размер еще больше
                $maxWidth = min($maxWidth, 800);
                $maxHeight = min($maxHeight, 800);
                // Для PNG качество работает по-другому, но мы все равно применяем resize
            }
