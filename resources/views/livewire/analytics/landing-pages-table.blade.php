<div>
    ...
    <div class="overflow-x-auto">
        <table class="w-full">
            <thead>
                <tr class="border-b">
                    <th class="text-left p-2">Landing Page URL</th>
                    <th class="text-right p-2">Visits</th>
            ...
            <tbody>
                @forelse($pages as $page)
                    <tr class="border-b">
                        <td class="p-2">
                            <a href="{{ $page['url'] ?? '#' }}" target="_blank" class="text-primary-600 hover:underline">
                                {{ $page['url'] ?? 'N/A' }}
                            </a>
                        </td>
                        <td class="p-2 text-right">{{ number_format($page['sessions'] ?? 0) }}</td>
                        <td class="p-2 text-right">{{ number_format($page['users'] ?? 0) }}</td>
                        <td class="p-2 text-right">{{ number_format(($page['bounce_rate'] ?? 0) * 100, 2) }}%</td>
                        <td class="p-2 text-right">{{ $page['avg_duration'] ?? '0:00' }}</td>
                    </tr>
                @empty
                    <tr>
