  ...
  @else
    @if($resource === 'comment')
      ...
      <div class="flex flex-col gap-6">
        {{-- Header --}}
        <div class="text-2xl font-semibold text-black mb-2">Report this Comment</div>
        ...
        <form wire:submit.prevent="submit" class="flex flex-col gap-4">
          <div class="flex flex-col gap-2">
            <label class="text-base font-semibold text-black">Tell us what's wrong with this comment</label>
            
            <div class="flex flex-col gap-3">
              <label class="flex items-center justify-between p-3 rounded border @error('form.reason') border-red-500 @else border-gray/30 @enderror hover:border-active cursor-pointer transition">
                <span class="text-sm text-black">Spam or Scam</span>
                <input type="radio" wire:model="form.reason" value="spam" name="report_reason" class="w-5 h-5 text-active focus:ring-active">
              </label>
              
              <label class="flex items-center justify-between p-3 rounded border @error('form.reason') border-red-500 @else border-gray/30 @enderror hover:border-active cursor-pointer transition">
                <span class="text-sm text-black">Offensive or abusive</span>
                <input type="radio" wire:model="form.reason" value="offensive" name="report_reason" class="w-5 h-5 text-active focus:ring-active">
              </label>
              
              <label class="flex items-center justify-between p-3 rounded border @error('form.reason') border-red-500 @else border-gray/30 @enderror hover:border-active cursor-pointer transition">
                <span class="text-sm text-black">Inappropriate content</span>
                <input type="radio" wire:model="form.reason" value="inappropriate" name="report_reason" class="w-5 h-5 text-active focus:ring-active">
              </label>
            </div>
